...
namespace MosadAPIServer.Services
{
	public class MissionService
	{
		...
		public MissionService(MosadDbContext context, AgentService agentService)
		{
			_context = context;
		...
		public async Task<Mission> UpdateMissionStatus(int id)
		{
			...
			if (IfMission(mission.AgentId, mission.TargetId) && await _agentService.IfNotTarget(mission.TargetId.Id))
			{
				mission.Status = MissionStatus.Status.InMission.ToString();
				mission.AgentId.Status = AgentStatus.Status.Active.ToString();
				RemoveMission(mission.AgentId.Id);
